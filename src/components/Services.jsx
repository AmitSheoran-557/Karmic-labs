...
const Services = () => {
    return (
        <div className='relative max-md:pt-3 max-lg:pb-6'>
            ...
            <div className="w-full mx-auto max-w-[1145px] max-xl:px-1">
                ...
                <div className="flex flex-wrap w-full max-lg:justify-center">
                    {SERVICES_CARDS_LIST.map((obj, index) => (
                        <div className="lg:w-4/12 md:w-6/12 w-full px-[11px] my-3 flex justify-center" key={index}>
                            <div className='max-w-[369px] w-full group relative'>
                                <div className='xl:size-[74px] bg-white rounded-full lg:size-16 size-14 group-hover:block hidden transition-all ease-linear duration-300 absolute -z-50 xl:-top-5 lg:-top-5 top-0 xl:-right-6 lg:-right-3 right-0'></div>
                                <div className="lg:min-h-[276px] sm:min-h-[229px] lg:py-5 py-3 lg:px-5 md:px-4 px-3 border border-transparent hover:border-white relative rounded-xl hover:border-opacity-[10%] transition-all ease-linear duration-300 hover:shadow-custom-one hover:backdrop-blur-30   hover:bg-white hover:bg-opacity-10 text-white">
                                    <div className='z-50 relative'>
                                        <span>{obj.image}</span>
                                        <h4 className='font-space font-bold lg:text-xl lg:mt-5 mt-4 text-base lg:mb-1.5 mb-1'>{obj.title}</h4>
                                        <p className='font-space lg:text-base text-sm max-w-[329px]'>{obj.description}</p>
